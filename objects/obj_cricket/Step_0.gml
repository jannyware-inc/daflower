...
if (state == enemy_states.alive){
	...
	switch(substate){
		case cricket_states.idle:
			xvel = 0;
			if(substate_timeup == 12){
				image_xscale = random(1) < .5 ? -1 : 1;
			}
			if(substate_timeup >= 25){
				substate_timeup = 0;
				substate = cricket_states.jumping;
			} else {
				substate_timeup++;
			}
			break;
		case cricket_states.jumping:
			if(xcollided != 0) image_xscale = -image_xscale;
			if(substate_timeup == 0 && grounded){
				xvel = sign(image_xscale) * 1.5;
				yvel = -4;
			}
			if(substate_timeup >= 2 && grounded){
				substate_timeup = 0;
